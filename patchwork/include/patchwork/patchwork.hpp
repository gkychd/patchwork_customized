...
#define UPRIGHT_ENOUGH 0.55 // cyan 青色
...
public:
    ...
    typedef std::vector<pcl::PointCloud<PointT> > Ring;//每个Ring内有一定数量的点
    typedef std::vector<Ring>                     Zone;//一共有4个Zone，每个Zone中有一定数量的Ring
    ...
    PatchWork(ros::NodeHandle *nh) : node_handle_(*nh) {
        ...
        node_handle_.getParam("/patchwork/czm/elevation_thresholds", elevation_thr_);//[0.523, 0.746, 0.879, 1.125]
        node_handle_.getParam("/patchwork/czm/flatness_thresholds", flatness_thr_);//[0.0005, 0.000725, 0.001, 0.001]
...
private:
    ...
    float           d_;//Ax+By+Cz+D=0中的D
    MatrixXf        normal_;//平面的 法向量
    ...
    vector<int> num_sectors_each_zone_;//划分的4个区域包含的sectors和rings都不同  [16, 32 ,54, 32]
    vector<int> num_rings_each_zone_;//[2, 4, 4, 4]
...
void PatchWork<PointT>::estimate_plane_(const pcl::PointCloud<PointT> &ground) {
    pcl::computeMeanAndCovarianceMatrix(ground, cov_, pc_mean_);//计算点云的协方差矩阵 以及三维质心（齐次形式）
    // Singular Value Decomposition: SVD
    Eigen::JacobiSVD<Eigen::MatrixXf> svd(cov_, Eigen::DecompositionOptions::ComputeFullU);//用SVD计算特征值和特征向量
    singular_values_ = svd.singularValues();

    // use the least singular vector as normal
    normal_ = (svd.matrixU().col(2));//法向量
    // mean ground seeds value
    Eigen::Vector3f seeds_mean = pc_mean_.head<3>();//取质心点的（x,y,z）

    // according to normal.T*[x,y,z] = -d
    d_         = -(normal_.transpose() * seeds_mean)(0, 0);//已知平面的法向量和中心点的坐标 求Ax+By+Cz+D=0中的D
    // set distance threhold to `th_dist - d`
    th_dist_d_ = th_dist_ - d_;//the_dist_ = 0.125  这里th_dist使得阈值更高一些 比如从-1.7 到 -1.6 ， th_dist越大会使越多的点被认为是地面点
}
...
void PatchWork<PointT>::extract_initial_seeds_(
    ...
    //由于本代码目前并没有将p_sorted的点云按高度排序，因此is_h_available设置为了false，也就是不进行使用预高度的操作
    //如果想该这一部分代码，可以尝试这么做，但个人感觉意义不大。
    if (is_h_available) {//是否需要利用已知的传感器高度比如kitti中的为1.73
        static double lowest_h_margin_in_close_zone = (sensor_height_ == 0.0) ? -0.1 : adaptive_seed_selection_margin_ *   //-1.1
                                                                                       sensor_height_;
        if (zone_idx == 0) {
    ...
    for (int i = 0; i < p_sorted.points.size(); i++) {
        if (p_sorted.points[i].z < lpr_height + th_seeds_) { //th_seeds = 0.5  取z的值比平均高度高0.5的点作为种子点
...
double PatchWork<PointT>::consensus_set_based_height_estimation(const Eigen::RowVectorXd& X,
    ...
    for (size_t i= 0 ;i < N ;++i){
        ...
        h.push_back(std::make_pair(X(i) + ranges(i), -i-1));
    }//利用range进行补偿，逻辑就是，该平面越平，补偿的值越小

    // ascending order
    std::sort(h.begin(), h.end(), [](std::pair<double, int> a, std::pair<double, int> b) { return a.first < b.first; });//从小到大排列

    int nr_centers = 2 * N;
    Eigen::RowVectorXd x_hat = Eigen::MatrixXd::Zero(1, nr_centers);
    Eigen::RowVectorXd x_cost = Eigen::MatrixXd::Zero(1, nr_centers);
    //以下的逻辑暂未找到理论依据
    double ranges_inverse_sum = ranges.sum();//所有值求和
    ...
    for (size_t i = 0 ; i < nr_centers ; ++i){
        ...
        int epsilon = (h.at(i).second > 0) ? 1 : -1;//该值为1或-1
        ...
        x_hat(i) = dot_X_weights / dot_weights_consensus;//考虑权值后的高度X

        double residual = consensus_set_cardinal * x_hat(i) * x_hat(i) + sum_xi_square  - 2 * sum_xi * x_hat(i);//consensus_set_cardinal越大说明只取了一边较多，因此会增大其残差值
    ...
    size_t min_idx;
    x_cost.minCoeff(&min_idx);//返回x_cost中最小的元素
...
void PatchWork<PointT>::estimate_sensor_height(pcl::PointCloud<PointT> cloud_in) {
    ...
    Ring ring_for_ATAT(num_sectors_for_ATAT_);//将一帧点云分成20个扇形，每份18度
    for (auto const &pt : cloud_in.points) {
        ...
        if ((r <= max_r_for_ATAT_) && (r > min_range_)) {//最大5    /    最小2.7
            double theta = xy2theta(pt.x, pt.y);

            sector_idx = min(static_cast<int>((theta / sector_size_for_ATAT)), num_sectors_for_ATAT_);//计算所属扇区索引
            ring_for_ATAT[sector_idx].points.emplace_back(pt);//向每个扇区添加所属点        圆环：大圆半径为5 小圆半径为2.7
        }
    }
    ...
    for (int i = 0; i < num_sectors_for_ATAT_; ++i) {
        ...
        if(ring_for_ATAT[i].size() < num_min_pts_ ){ continue; }//每个扇区中至少要有10个点

        pcl::PointCloud<PointT> dummy_est_ground;//地面点
        pcl::PointCloud<PointT> dummy_est_non_ground;//非地面点
        extract_piecewiseground(0, ring_for_ATAT[i], dummy_est_ground, dummy_est_non_ground, false);//该函数不仅得到地面点和非地面点，也得到了拟合平面的各项参数
        ...
        if (ground_z_vec > uprightness_thr_ && linearity < 0.9) { //uprightness_thr_ = 0.707（45度） /   0.866（60度）
    ...
    Eigen::Matrix<double, 1, Eigen::Dynamic> ranges = noise_bound_ * Eigen::MatrixXd::Ones(1, N);//noise_bound_ = 0.2
    Eigen::Matrix<double, 1, Eigen::Dynamic> weights = 1.0 / (noise_bound_ * noise_bound_) * Eigen::MatrixXd::Ones(1, N);//1.0 / (noise_bound_ * noise_bound_) = 25
    for (int i = 0; i < N; ++i) {
...
void PatchWork<PointT>::estimate_ground(
    ...
    if (!poly_list_.polygons.empty()) poly_list_.polygons.clear();//设置点
    if (!poly_list_.likelihood.empty()) poly_list_.likelihood.clear();//设置显示颜色
    ...
    // 22.05.02 Update
    // Global sorting is deprecated 全局排序弃用
//    sort(laserCloudIn.points.begin(), laserCloudIn.end(), point_z_cmp<PointT>);
    ...
    for (int k = 0; k < num_zones_; ++k) {//num_zones_ = 4 划分四个区域
    ...
    for (int k              = 0; k < num_zones_; ++k) {
        ...
        for (uint16_t ring_idx = 0; ring_idx < num_rings_each_zone_[k]; ++ring_idx) {
            for (uint16_t sector_idx = 0; sector_idx < num_sectors_each_zone_[k]; ++sector_idx) {
                if (zone[ring_idx][sector_idx].points.size() > num_min_pts_) {//num_min_pts_ = 10
                    double t_tmp0 = ros::Time::now().toSec();
                    // 22.05.02 update
                    // Region-wise sorting is adopted
                    sort(zone[ring_idx][sector_idx].points.begin(), zone[ring_idx][sector_idx].end(), point_z_cmp<PointT>);//每个bin中的点从小到大排序
                    extract_piecewiseground(k, zone[ring_idx][sector_idx], regionwise_ground_, regionwise_nonground_);//分别在每个bin中区分地面点和非地面点
                    ...
                    if (ground_z_vec < uprightness_thr_) {//0.707（45度） /  0.866（60度）
                        // All points are rejected
                        cloud_nonground += regionwise_ground_;
                        cloud_nonground += regionwise_nonground_;
                    } else { // satisfy uprightness
                        if (concentric_idx < num_rings_of_interest_) {//num_rings_of_interest_ = 4   在4个bin中最多只对4个ring进行以下处理
                            if (ground_z_elevation > -sensor_height_ + elevation_thr_[ring_idx + 2 * k]) { //elevation_thr_ = [0.523, 0.746, 0.879, 1.125]
...
void PatchWork<PointT>::extract_piecewiseground(
    ...
    // 1. set seeds!
    extract_initial_seeds_(zone_idx, src, ground_pc_, is_h_available);//ground_pc_为输出的初始种子点

    // 2. Extract ground
    for (int i = 0; i < num_iter_; i++) {
        estimate_plane_(ground_pc_);
        ground_pc_.clear();//利用完后释放内存
        ...
        // ground plane model
        Eigen::VectorXf result = points * normal_;// = -D
        // threshold filter
